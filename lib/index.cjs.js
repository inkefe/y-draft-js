...
var DraftBinding = function DraftBinding(opts) {
  ...
  this.setStateByRaw = function (raw) {
    ...
    var newEditorState = draftJs.EditorState.set(draftJs.EditorState.createWithContent(draftJs.convertFromRaw(raw)), {
  ...
  this.setStateAndSelection = function (_onChange, newEditorState, isCollapsed, raw) {
    ...
    var _ref = _this.selectData || _this.getSectionData(),
        anchorKey = _ref.anchorKey,
        focusKey = _ref.focusKey,
  ...
  this.destroy = function () {
    ...
    provider.off('status', _this.onStatusChange);
  ...
  this.ymap = ymap;
  this.awareness = _provider.awareness;
  this._lock = false;
  this._updatable = !!updatable;
  this.isConnecting = _provider.wsconnecting;

  this.onStatusChange = function (_ref2) {
    var status = _ref2.status;
    _this.isConnecting = status === 'connecting';
  };

  _provider.on('status', this.onStatusChange);
  ...
  !_provider.synced ? _provider.on('sync', function (isSynced) {
    if (!isSynced) return;
