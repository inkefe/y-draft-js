...
var _throttle__default = /*#__PURE__*/_interopDefaultLegacy(_throttle);
...
var DMP = new Dmp__default["default"](); // 计算序号, 默认不跟随内容更改
...
function toSyncElement(item) {
  ...
  if (Array.isArray(item)) {
    return Y__namespace.Array.from(item.map(toSyncElement));
  }
...
var _pathTargeMap = new WeakMap(); // 获取指定路径的数据，如果有值则用回调返回，如果没有则自动监听到目标值出现, 并且持续监听目标路径下的y对象，一旦被更改成另一个对象也会执行回调，并返回的cancle方法来取消监听


var onTargetSync = function onTargetSync(path, ymap, cb) {
  var _targetMap$pathKey;

  if (!ymap) return console.warn('ymap is undefined');
  if (!cb) return console.warn('callback is necessary in onTargetSync');
  var targetMap = _pathTargeMap.has(ymap) ? _pathTargeMap.get(ymap) || {} : {};
  ...
  targetMap[pathKey] = {
    target: target,
    callBacks: [].concat(((_targetMap$pathKey = targetMap[pathKey]) == null ? void 0 : _targetMap$pathKey.callBacks) || [], [cb])
  };

  function ob(e) {
    var target = getTargetByPath(path, e[0].currentTarget, true);
    if (!target || !targetMap[pathKey] || targetMap[pathKey].target === target) return; // 等待目标字段的内容出现

    targetMap[pathKey].target = target;
    targetMap[pathKey].callBacks.forEach(function (fn) {
      return fn(target);
    });
  }
  ...
  ymap.observeDeep(ob);
  return function () {
    targetMap[pathKey].callBacks.splice(targetMap[pathKey].callBacks.indexOf(cb), 1);
...
var DraftBinding = function DraftBinding(opts) {
  ...
  this.updateAwareness = _throttle__default["default"](function (selectData) {
    _this.awareness.setLocalStateField('selection', selectData);
  }, 300);
  ...
  this.forceRefresh = function (currentTarget) {
    var raw = rbw2raw(currentTarget.toJSON());
  ...
  this.listenTargetYmap = function (rawYmap) {
    ...
    if (isRaw(val)) {
      ...
      _this.log('rawYmap :[error]', rawYmap.toJSON(), val, _this.rawPath.join('.'));
    ...
    if (!_this.oprID) {
      _this.oprID = draftJs.genKey();

      _this.rawYmap.set(CHANGE_CLIENT, _this.oprID);
    }

    _this.undoManager = new Y__namespace.UndoManager(_this.rawYmap, {
  ...
  this.bindEditor = function (editor) {
    ...
    var draftEditor = editor.update ? editor : editor.editor; // 原始的draft-editor
    ...
    if (isPluginEditor) {
      ...
      _this._onChange && (editor.onChange = function (editorState) {
        ...
        if (that._lockFocus) {
          editorState = draftJs.EditorState.forceSelection(editorState, that.newSelection);
        }
  ...
  this.setStateByRaw = function (raw) {
    ...
    var newEditorState = draftJs.EditorState.createWithContent(draftJs.convertFromRaw(raw));
    ...
    if (Object.values(editorMap).filter(Boolean).length > 0 && !editorMap[_this.editorKey]) {
  ...
  this.setStateAndSelection = function (_onChange, newEditorState, isCollapsed, raw) {
    ...
    var hasFocus = selectionState.getHasFocus();
    ...
    _this.editorState = newSelection ? draftJs.EditorState[isCollapsed || _this.shouldAcceptSelection ? 'acceptSelection' : 'forceSelection'](newEditorState, newSelection) : newEditorState;
  ...
  this.destroy = function () {
    ...
    _this.rawYmap && _this.rawYmap.unobserveDeep(_this.onObserveDeep);

    if (_this.awareness !== null) {
      // @ts-ignore
      _this.awareness.off('change', _this.rerenderDecorations);
    }
  };
  ...
  this.onObserveDeep = function (events) {
    ...
    var originOrpId = events[0].currentTarget.get(CHANGE_CLIENT);
  ...
  this.onChange = function (editorState) {
    return _this.mutex(function () {
      ...
      if (!_this._updatable) return;
      var selection = editorState.getSelection(); // console.log(selection.serialize());
      ...
      _this.updateAwareness(selectData);

      var newJson = JSON.stringify(raw);
      var rawYmap = getTargetByPath(_this.rawPath, _this.ymap);
      if (!rawYmap) return;
...
exports.getNewSelection = getNewSelection;
