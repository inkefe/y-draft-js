...
var onTargetSync = function onTargetSync(path, ydoc, cb, firstType) {
  ...
  var firstData = ydoc.get(fixField, Y__namespace.Map);
  ...
  return function () {
    targetDoc[pathKey].callBacks.splice(targetDoc[pathKey].callBacks.indexOf(cb), 1);
...
var setRawToSharedData = function setRawToSharedData(rawPath, ydoc, raw) {
  ...
  ydoc.transact(function () {
    ...
    target.set(CHANGE_CLIENT, draftJs.genKey()); // const rawYmap = toRawSharedData(rbw)
...
var DraftBinding = function DraftBinding(opts) {
  ...
  this.bindEditor = function (editor) {
    ...
    var draftEditor = editor.update ? editor : editor.getEditorRef(); // 原始的draft-editor
  ...
  this.setStateByRaw = function (raw) {
    ...
    if (_this.isPluginEditor && !_this.draftEditor.getEditorRef()) {
      return console.warn('drafjsEditor Can`t be found it, please rebind the editor', _this.draftEditor);
    }
...
exports.setRawToSharedData = setRawToSharedData;
