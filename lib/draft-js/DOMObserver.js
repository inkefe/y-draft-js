...
var DOMObserver = /*#__PURE__*/function () {
  function DOMObserver(container) {
    ...
    this.mutations = Map(); // this._mutations = [];

    var containerWindow = getWindowForNode(container);
    var MutationObserver = containerWindow.MutationObserver;
  ...
  _proto.start = function start(editor) {
    var lastEditorState = editor._latestEditorState;
    var selection = lastEditorState.getSelection();
  ...
  _proto.registerMutation = function registerMutation(mutation) {
    ...
    if (textContent != null) {
      var offsetKey = findAncestorOffsetKey(mutation.target);
      if (!offsetKey) return;
      var blockKey = offsetKey.split('-')[0];
      this.starKey = this.starKey || blockKey;
      if (this.starKey !== blockKey) return; // this._mutations.push(mutation);
